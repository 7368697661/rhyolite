...
import {
  ...
  saveTimelineScope,
} from "@/app/api/timeline/resolveScope";
import { findNodeScope } from "@/app/api/timeline/nodes/scope";
...
const readTimelineTool: AgentTool = {
  ...
  async execute(args, ctx) {
    ...
    const scope = await resolveTimelineScope(null, tid);
    if (!scope)
...
const createTimelineNode: AgentTool = {
  ...
  async execute(args, ctx) {
    ...
    const scope = await resolveTimelineScope(did, tid);
...
const createEdge: AgentTool = {
  ...
  async execute(args, ctx) {
    ...
    const scope = await findNodeScope(sourceId);
...
const deleteEdge: AgentTool = {
  ...
  async execute(args, ctx) {
    ...
    const projects = (await import("./fs-db")).listProjects;
    const { listTimelines: lt } = await import("./fs-db");
...
const resolveDeadLinks: AgentTool = {
  ...
  async execute(args, ctx) {
    ...
    const findSpecialist = (role: string): FsGlyph | null =>
      allGlyphs.find(g => g.isSculpter === false && g.specialistRole?.toLowerCase() === role) || null;

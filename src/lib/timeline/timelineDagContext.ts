import { listProjects, listTimelines, listDocuments, readDocument, readWikiEntry, listWikiEntries, type FsTimelineEvent, type FsEventEdge, type FsDocument, type FsWikiEntry } from "../agents/fs-db";
...
function getAncestorSubgraph(
  ...
  const relevantEdges = new Set<{ source: string; target: string; label?: string | null }>();
...
async function getFullTimelineGraph(nodeId: string): Promise<{ activeNode: FsTimelineEvent, allNodes: FsTimelineEvent[], allEdges: FsEventEdge[], projectId: string } | null> {
  ...
  for (const p of projects) {
    ...
    for (const t of timelines) {
      const node = t.events.find((e: any) => e.id === nodeId);
      if (node) return { activeNode: node, allNodes: t.events, allEdges: t.edges, projectId: p.id };
    }
    const docs = await listDocuments(p.id);
    for (const d of docs) {
      const data: any = null; // await readDocumentTimeline(p.id, d.id);
      if (!data) continue;
      const node = data.events.find((e: any) => e.id === nodeId);
      if (node) return { activeNode: node, allNodes: data.events, allEdges: data.edges, projectId: p.id };
    }
  }
  return null;
}
...
export async function resolveChatProjectContext(
  ...
  for (const p of projects) {
    if (chat.documentId) {
      const docs = await listDocuments(p.id);
      const document = docs.find((d: any) => d.id === chat.documentId);
      if (document) {
      ...
      const timelines = await listTimelines(p.id);
      const timeline = timelines.find((t: any) => t.id === chat.timelineId);
      if (timeline) {
        const wikiEntries = await listWikiEntries(p.id);

...
export function extendTipFromModel(
  ...
  while (true) {
    ...
    tip = u.id;
    const models = (children.get(tip) ?? []).filter((k) => k.role === "model" || k.role === "assistant");
...
export function siblingModelsForUser(
  ...
  return (children.get(userId) ?? []).filter((m) => m.role === "model" || m.role === "assistant");
}
